...
class Content extends Component {
    ...
    sbros = () => {
        ...
        document.querySelector('#operation').textContent = 'Все адреса';
        setTimeout(() => {
            this.setState({
                detail: false,
                isActivyears: false,
        ...
        setTimeout(this.clickselect0(), 100);
    };
    ...
    clickselect1 = (date) => {
        ...
        if (this.state.date1) {
            if (this.state.data_time == 'за год') {
                this.setState({
                    list_content: this.state.list_content.filter(
                        (a) =>
                            this.state.date1.getFullYear() ==
                            new Date(a.dates).getFullYear()
                    ),
                });
            }
            if (this.state.data_time == 'за месяц') {
                this.setState({
                    list_content: this.state.list_content.filter(
                        (a) =>
                            this.state.date1.getMonth() + 1 ==
                                new Date(a.dates).getDate() &&
                            this.state.date1.getFullYear() ==
                                new Date(a.dates).getFullYear()
                    ),
                });
    ...
    dop2() {
        setTimeout(() => {
            ...
            if (
                this.state.select_oper != 'Все адреса' &&
                this.state.select_oper != 'Прочие адреса'
            ) {
                this.setState({
                    list_content: this.state.account.filter(
                        (a) => a.addr_account.name == this.state.select_oper
                    ),
                });
                ...
                this.state.name_adr.map((item, index) => {
                    if (index > 4)
                        this.setState({
                            list_content: this.state.account.filter(
                                (a) => a.addr_account.name == item.name
                            ),
                        });
                });
    ...
    componentDidMount() {
        ...
        fetch('orders.json')
            ...
            .then((response) => {
                this.setState({ account: response.orders });
            ...
            .finally(() => {
                this.setState({ list_content: this.state.account });
                this.clickselect2();
                this.detail_click();
            });
    ...
    render() {
        ...
        const currentPost = this.state.list_content.slice(
            IndexofFirstPost,
            IndexofLastPost
        ); //Посты к выгрузке
                ...
                <div className='inner-header-content'>
                    ...
                    <div style={{ marginRight: '3vw' }}>
                        <div className='positionselect'>
                            <Selecting
                                onChange={this.clickselect0}
                                data_time={this.state.data_time}
                            />
                        </div>
                    ...
                    <div className='text'>адрес доставки:</div>
                    <div className='positionselect'>
                        <Selecting
                            name_adr={this.state.name_adr}
                            onChange={this.clickselect2}
                            data={this.state.select_oper}
                            open={() => this.openisActivyears()}
                        />
                    </div>
                        ...
                        {this.state.detail && (
                            <div>
                                {currentPost.map((item, index) => {
                                    return (
                                        <Contents
                                            ...
                                            person_discount={
                                                item.data.person_discount
                                            }
                                            discount_poll={
                                                item.data.discount_poll
                                            }
                                            rate_delivery={
                                                item.data.rate_delivery
                                            }
                                            summ_bonus={item.data.summ_bonus}
                                            products_list={
                                                item.data.products_list
                                            }
                                            payment_methods={
                                                item.data.payment_methods
                                            }
                                ...
                                <div>
                                    <Pagination
                                        totalPost={
                                            this.state.list_content.length
                                        }
                                        postPerPage={this.state.postPerPage}
                                        paginete={this.paginete}
