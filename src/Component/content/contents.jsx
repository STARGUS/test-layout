...
function Contents(props) {
    ...
    const [product_summ, setProduct_summ] = useState(0); // запись суммы продуктов для каждого заказа
    const [summ_order, setSumm_order] = useState(0);
    const [productshow, setProductshow] = useState(false);
    const [MoDorate, setMoDorate] = useState('');

    const Ps = (ite) => {
        // расчет суммы продуктов по умолчанию или при изменении
        let newItem = 0;
        if (ite) {
            ite.map((i) => (newItem += i.price * i.count));
        } else {
            count.map((i) => (newItem += i.price * i.count));
        }
        ...
        setSumm_order(
            (
                newItem -
                (rate_bonus ? rate_bonus : 0) -
                ((newItem - (rate_bonus ? rate_bonus : 0)) *
                    (person_discount + discount_poll)) /
                    100 +
                rate_delivery
            ).toFixed()
        );
    ...
    let summ_orders = (
        product_summs -
        (rate_bonus ? rate_bonus : 0) -
        ((product_summs - (rate_bonus ? rate_bonus : 0)) *
            (person_discount + discount_poll)) /
            100 +
        rate_delivery
    ).toFixed();
        ...
        <div typeof={types} key={props.key} className={wrapper}>
            ...
            >
                <div className='accordion-titel'>
                    <div className={stile}>{!isActive && <>&#9655;</>}</div>
                </div>

                <>
                    <span>{order}</span>
                    <span>{dates}</span>
                    <span className='rate-bonus-style'>
                        {coming_bonus && <>{coming_bonus}&#x263B;</>}
                    </span>
                    <div>
                        <div className='rate-style'>
                            -{summ_order > 0 ? summ_order : summ_orders} &#8381;
                        </div>
                        {rate_bonus && (
                            <div className='rate-bonus-style rate-bonus-left'>
                                -{rate_bonus}&#x263B;
                            </div>
                        )}
                    </div>
                ...
                {isActive && (
                    ...
                    >
                        ...
                        <div>
                            {tel.map((item, index) =>
                                !tel.length == index ? (
                                    <div className='tel' key={index + 'tel1'}>
                                        {item}
                                    </div>
                                ) : (
                                    <div key={index + 'tel2'}>{item}</div>
                                )
                            )}
                        </div>
                        ...
                        <div>
                            {withing[0]}
                            <span className='up'>{withing[1]}</span> &#8212;
                            {befores[0]}
                            <span className='up'>{befores[1]}</span>
                        </div>
                        <div>Комментарий</div>
                        <div>
                            {comment.map((item, index) => (
                                <div key={index + 'comment'}>{item}</div>
                            ))}
                        </div>
                        <div>Способы оплаты</div>
                        <div>{payment_methods}</div>
                        <div className='item_prods' onClick={() => products()}>
                            Список товаров
                        </div>
                        <div></div>
                        <ReactCSSTransitionGroup
                        ...
                        <div className={MoDorate + 'size1_1'}>
                            {product_summ > 0 ? product_summ : product_summs}{' '}
                            &#8381;
                        </div>
                        <div>Оплачено балами</div>
                        <div className='rate-bonus size1_1'>
                            {rate_bonus ? <>{rate_bonus}&#x263B;</> : 0}
                        </div>
                        ...
                        <div className='size1_4_bold'>
                            <div className={MoDorate + 'pluss1'}>
                                {summ_order > 0 ? summ_order : summ_orders}{' '}
                                &#8381;
                            </div>{' '}
                            <div className='plus_add plus_add_1'>+</div>{' '}
                            <div className='povtororder fontdecor'>
                                Повторить весь заказ
                            </div>
                        </div>
                        <div>Бонус балов за заказ</div>
                        <div className='rate-bonus size1_1'>
                            {coming_bonus ? <>{coming_bonus}&#x263B;</> : 0}
                        </div>
                    </div>
                )}
            </ReactCSSTransitionGroup>
